...
class AutoReportGeneratorCog(commands.Cog):
    ...
    async def generate_report(self, ctx, report_type: str, *, description: str):
        ...
        if not await self.is_staff(ctx):
            await ctx.send("❌ Only staff can generate reports.")
            return
    ...
    async def schedule_report(self, ctx, report_type: str, frequency: str, *, template: str):
        ...
        if not await self.is_staff(ctx):
            await ctx.send("❌ Only staff can schedule reports.")
            return
        ...
        await ctx.send(f"✅ Report scheduled: {report_type} ({frequency})")
    ...
    async def report_detail(self, ctx, report_id: int):
        ...
        if not report:
            await ctx.send(f"❌ Report RPT-{report_id} not found.")
            return
    ...
    async def list_reports(self, ctx, limit: int = 10):
        ...
        embed = discord.Embed(title="📊 Recent Reports", color=discord.Color.blue())
